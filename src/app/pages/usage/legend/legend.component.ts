import {Component, OnInit, HostListener, Input, Output, ViewEncapsulation} from '@angular/core';
...
export class LegendComponent implements OnInit {
    ...
    destroyLegend() {
        ...
        this.options.dictList = null;
        this.options.buildData = null;
        this.options.curCoords = null;
    ...
    setLegendSrc(src = '', fnImgLoaded) {
        ...
        let img = new Image();
        let onLegendBeforeLoad = this.callbacks.onLegendBeforeLoad;
        let onLegendAfterLoad = this.callbacks.onLegendAfterLoad;

        if (typeof onLegendBeforeLoad === "function") {
            onLegendBeforeLoad.apply(this, []);
        }

        img.onload = function () {
            if (typeof onLegendAfterLoad === "function") {
                onLegendAfterLoad.apply(this, [img]);
            }
            this.finishImgLoad(img, fnImgLoaded);
        };
    ...
    redraw() {
        ...
        if (this.curCoords.length > 0) {
            this.clearHotpoint();
            this.activateHotpoint(this.curCoords, this.curCallouts);
        }
    }
